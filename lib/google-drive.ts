...
export class GoogleDriveService {
   ...
   */
  async listFiles(folderId?: string, pageSize = 1000, searchQuery?: string) {
    try {
      console.log('📋 Google Drive listFiles called...');
      ...
      if (searchQuery && searchQuery.trim()) {
        query += ` and name contains '${searchQuery.trim()}'`;
      }
      ...
      console.error('❌ Error listing files in Google Drive service:', error);
